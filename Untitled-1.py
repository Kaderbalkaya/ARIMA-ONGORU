...
output_size = 6        # sonraki 4 degeri tahmin etmeye calis
...
raw_data = pd.read_csv(r"C:\Users\kader\OneDrive\Masaüstü\CV\wheat1-1.csv", names=["Year", "Production"], sep=';')
Year = np.array(raw_data.Year.values)[1:]
Production = np.array(raw_data.Production.values[1:], dtype=np.float64)


min = Production.min()
max = Production.max()

y =np.interp(Production, (min, max), (-1, +1))
...
##plt.plot(Year,output_size, c='green')
##plt.show(reslt)